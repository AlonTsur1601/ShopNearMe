...
export default defineConfig(({ mode }) => {
  ...
  return {
  plugins: [react(), { name: "shopnearme-search-api", configureServer(server) { server.middlewares.use("/api/search", async (request, response) => { const url = new URL(request.url ?? "", "http://localhost"); const query = url.searchParams.get("q")?.trim() ?? ""; const location = url.searchParams.get("location")?.trim() ?? ""; const scope = url.searchParams.get("scope") ?? "all"; const rawLat = url.searchParams.get("lat"); const rawLon = url.searchParams.get("lon"); const lat = Number(rawLat); const lon = Number(rawLon); const coordinates = rawLat !== null && rawLon !== null && Number.isFinite(lat) && Number.isFinite(lon) ? { lat, lon } : undefined; response.setHeader("Content-Type", "application/json"); if (!query) { response.statusCode = 400; response.end(JSON.stringify({ error: "A product query is required" })); return; } try { response.end(JSON.stringify(await searchRetailCatalog(query, location, { provider: "octoparse", octoparseApiKey: env.OCTOPARSE_API_KEY, continuation: url.searchParams.get("continuation") || "" }, coordinates, { clientId: env.EBAY_CLIENT_ID, clientSecret: env.EBAY_CLIENT_SECRET }, scope))); } catch (error) { response.statusCode = 502; response.end(JSON.stringify(publicSearchError(error))); } }); } }],
  server: {
    host: "127.0.0.1",
    port: 4173,
  },
