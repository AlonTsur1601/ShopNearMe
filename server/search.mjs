...
function attributesFor(query, text, condition, merchant, meta = {}) { text = `${text} ${meta.specificationText ?? ""}`; const attributes = { condition, retailer: shortRetailerName(merchant) }; for (const rule of rulesFor(query)) { if (attributes[rule.id]) continue; const value = rule.infer ? rule.infer(text, meta) : inferValue(text, rule.values); if (value) attributes[rule.id] = value; } return attributes; }
...
async function enrichOffer(offer, query) {
  if (!offer?.destinationUrl || (offer.imageUrl && offer.itemPrice !== null)) return offer;
  ...
  return { ...offer, title: page.title && isRelevantProduct(page.title, query) ? page.title : offer.title, imageUrl: offer.imageUrl || page.imageUrl || "", itemPrice, totalPrice: itemPrice === null ? null : shippingPrice === null ? itemPrice : itemPrice + shippingPrice, currency: offer.itemPrice === null && page.price !== null ? page.currency : offer.currency, priceVerified: itemPrice !== null, attributes: { ...attributesFor(query, `${offer.title} ${offer.subtitle} ${page.title ?? ""}`, offer.condition, offer.merchant, page), ...offer.attributes } };
}
...
function nearbyProductOffers(maps, offers) {
  ...
  })).map(offer => ({ ...offer, id: `${offer.id}-pickup`, category: "local", totalPrice: offer.itemPrice, shippingPrice: null, availability: "Product listed · check branch stock" }));
}
...
export function buildFacets(offers, query) {
  ...
  const definitions = [["condition", "Condition"], ...rulesFor(query).map(({ id, label }) => [id, label]), ["retailer", "Retailer"]];
  return definitions.filter(([id], index) => definitions.findIndex(([other]) => other === id) === index).map(([id, label]) => {
    const counts = new Map();
    let covered = 0;
    for (const offer of offers) { const value = offer.attributes[id]; if (value) { covered++; counts.set(value, (counts.get(value) ?? 0) + 1); } }
    const specific = specificIds.has(id);
    const minimum = Math.max(2, Math.ceil(offers.length * (specific ? .05 : .2)));
    if (!counts.size || (counts.size === 1 ? !specific || covered === offers.length : covered < minimum)) return null;
    return { id, label, options: [...counts].sort((a, b) => b[1] - a[1]).slice(0, 12).map(([value, count]) => ({ value, count })) };
  }).filter(Boolean);
}
...
async function shoppingSearch(query, location, key) {
  ...
  const offers = rows.map((item, index) => {
    ...
    return { ...offer, shippingPrice, totalPrice: offer.itemPrice === null ? null : offer.itemPrice + (shippingPrice ?? 0), attributes: attributesFor(query, `${item.title} ${(item.extensions ?? []).join(" ")}`, offer.condition, offer.merchant, item) };
  }).filter(Boolean);
