...
function onlineStoreOffer(item, index, query) {
  ...
  return { id: `online-store-${index}-${new URL(destinationUrl).hostname}`, category: "order", merchant, merchantLogoUrl: safeHttpUrl(item.favicon), title: merchant, subtitle: String(item.snippet ?? "").slice(0, 150) || "Online retailer", imageUrl: "", rating: 0, reviewCount: 0, itemPrice: null, shippingPrice: null, totalPrice: null, currency: "USD", priceVerified: false, potentialStore: true, availability: "Potential online retailer · confirm product availability", attributes: { retailer: merchant }, destinationUrl, linkLabel: "View store" };
}
...
function completeFacetCohort(offers, required) {
  const products = offers.filter(offer => !offer.potentialStore);
  if (!products.length || !required.length) return offers;
  const minimum = Math.min(products.length, Math.max(2, Math.ceil(products.length * .2)));
  const active = required.filter(id => products.some(offer => valuesForFacet(offer, id).length));
  let complete = products.filter(offer => active.every(id => valuesForFacet(offer, id).length));
  while (complete.length < minimum && active.length > 1) {
    const coverage = active.map((id, index) => ({ id, index, count: products.filter(offer => valuesForFacet(offer, id).length).length }));
    const remove = coverage.sort((a, b) => a.count - b.count || b.index - a.index)[0];
    active.splice(active.indexOf(remove.id), 1);
    complete = products.filter(offer => active.every(id => valuesForFacet(offer, id).length));
  }
  const selected = new Set(complete);
  return offers.filter(offer => offer.potentialStore || selected.has(offer));
}
...
async function runScope(scope, query, location, key, coordinates, credentials) {
  ...
  const jobs = scope === "online" ? [shoppingSearch(query, location, key), localProductSearch(query, location, key), ebaySearch(query, location, credentials)]
    ...
    : [shoppingSearch(query, location, key), mapsSearch(query, location, key, coordinates), localProductSearch(query, location, key), ebaySearch(query, location, credentials)];
