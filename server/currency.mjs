...
async function rate(from, to) {
  ...
  try {
    const response = await fetch("https://api.frankfurter.dev/v2/rate/" + encodeURIComponent(from) + "/" + encodeURIComponent(to) + "?providers=ECB", { signal: searchSignal(controller.signal) });
