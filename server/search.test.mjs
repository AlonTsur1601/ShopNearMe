...
describe("product page enrichment", () => {
  it("reports explicit product stock without guessing when it is absent", () => {
    for (const [status, expected] of [["OutOfStock", "Out of stock"], ["InStock", ""], ["PreOrder", "Out of stock"], ["", ""]]) {
  ...
  it("builds useful dining-table facets and concise retailer names", async () => {
    vi.stubGlobal("fetch", vi.fn(async (url) => {
      ...
      if (request.searchParams.get("engine") === "google_shopping") return { ok: true, json: async () => ({ shopping_results: [
        { position: 1, title: "72 inch solid wood extendable dining table with 8 chairs", source: "SIHOO ישראל אתר היבואן הרשמי", extracted_price: 800, delivery: "Free shipping", product_link: "https://sihoo.example/products/table-8" },
        { position: 2, title: "Round glass fixed dining table with 4 chairs", source: "SIHOO ישראל אתר היבואן הרשמי", extracted_price: 650, delivery: "Free shipping", product_link: "https://sihoo.example/products/table-4" },
