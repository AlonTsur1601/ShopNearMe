...
export function registerShopNearMeTools(bridge: WebMCPBridge): () => void {
  ...
  register({ name: "filter_results", title: "Filter product results", description: "Apply product-specific facet filters. Values must match the facet options returned by search_products.", inputSchema: { type: "object", properties: { filters: { type: "object", additionalProperties: { type: "array", items: { type: "string" } } } }, required: ["filters"] }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: ({ filters }) => { bridge.setFilters((filters ?? {}) as Record<string, string[]>); return { applied: true }; } });
  register({ name: "sort_results", title: "Sort product results", description: "Sort current offers by total price (including shipping) or by distance from the selected location.", inputSchema: { type: "object", properties: { direction: { type: "string", enum: ["price-asc", "price-desc", "distance-asc", "distance-desc"] } }, required: ["direction"] }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: ({ direction }) => { bridge.setSort(direction as SortDirection); return { direction }; } });
