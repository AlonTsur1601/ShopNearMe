...
const labels: Record<OfferCategory, string> = { order: "Order online", local: "Nearby retailers", secondHand: "Second hand" };
...
export function OfferSection({ category, offers, distanceUnit }: { category: OfferCategory; offers: Offer[]; distanceUnit: DistanceUnit }) {
  ...
  return <section className="offer-section" aria-labelledby={`category-${category}`}><h2 id={`category-${category}`}><Icon size={21} />{labels[category]} <span>({offers.length})</span></h2><div className="offer-table-heading" aria-hidden="true"><span>Offer</span><span>Retailer</span><span>Variant / Specs</span><span>Availability</span><span>Total price</span><span>Details</span></div><div className="offer-list">{offers.map((offer) => <article className="offer-row" key={offer.id}><ProductImage key={offer.imageUrl} offer={offer} /><div className="offer-merchant"><div className="merchant-identity">{offer.merchantLogoUrl && <img src={offer.merchantLogoUrl} alt="" />}<a href={offer.destinationUrl} target="_blank" rel="noreferrer">{offer.merchant}</a></div>{offer.reviewCount > 0 && <span><Stars rating={offer.rating} /> <small>({offer.reviewCount.toLocaleString()})</small></span>}</div><div className="offer-specs"><strong>{offer.title}</strong><span>{offer.subtitle}</span>{offer.condition && !offer.subtitle.split(/\s*[·|]\s*/).some(part => part.trim().toLowerCase() === offer.condition?.trim().toLowerCase()) && <small>{offer.condition}</small>}</div><div className="offer-availability">{offer.availability && <strong className={/^out of stock$/i.test(offer.availability) ? "stock-unavailable" : undefined}>{offer.availability}</strong>}{category === "local" && <span className="pickup-label">{offer.pickupVerified ? "Local pickup" : "Check branch stock"}</span>}{offer.arrival && <small>{offer.arrival}</small>}{offer.distanceMiles !== undefined && <small>{displayDistance(offer.distanceMiles, distanceUnit)}</small>}</div><Price offer={offer} category={category} /><a className="secondary-button offer-link" href={offer.destinationUrl} target="_blank" rel="noreferrer">{offer.linkLabel ?? "View product"}</a></article>)}</div></section>;
}
